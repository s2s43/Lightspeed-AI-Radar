...
def main():
    ...
    # تنسيق الواجهة لتطابق الطابع الداكن لبرنامج Lightspeed القياسي
    st.markdown("""
        <style>
    ...
    # تفعيل ميزة التحليل على جميع الفريمات الزمنية المتعددة اللحظية والتاريخية
    ...
    # أسهم الترند الافتراضية لعرض الزخم الحركي تزامناً مع نوع السوق
    st.sidebar.markdown("---")
    ...
    if trigger_radar:
        ...
        # ربط الفريمات بالفترات الزمنية المناسبة تلقائياً لتفادي ثقل التحميل وقيم nan
        ...
        with st.spinner(f"📡 جاري الاتصال بالبورصة ومعالجة بيانات {ticker_resolved} فوريّاً..."):
            try:
                ...
                # تشغيل خوارزمية حساب مستويات الأهداف ومناطق الدخول الآمنة
                ...
                # === عرض لوحة فحص المؤشرات اللحظية الأساسية ===
                ...
                # === عرض منطقة الدخول ومصفوفة أهداف القناص الثلاثة ===
                ...
                with col_t2:
                    # ميزة الإشعارات والتحذيرات اللحظية المباشرة في نفس بيانات العمل
                    st.subheader("🔔 مركز الإشعارات الفورية ونصائح الرادار")
                    if last_vol > (hist['Volume'].mean() * 1.5):
                        st.error(f"⚡ **إشعار اختراق سيولة:** تم رصد تدفق حجم تداول ضخم ومفاجئ يفوق المعدل المعتاد بـ 150%! السيولة الحالية تفاعلية.")
                ...
                # === رسم شارت الشموع اليابانية التفاعلي الحقيقي للتحليل الفني وعمق الأسعار ===
                ...
                # دمج خطوط مستويات الخوارزمية والأهداف بصرياً على شاشة الشارت
